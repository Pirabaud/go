...
void BoardScene::drawDefaultTexts(sf::RenderWindow& window) const {
    ...
    stoneToPlay.setPosition({
        DisplayService::WINDOW_WIDTH - PADDING - 20, DisplayService::WINDOW_HEIGHT - PADDING - 20
    });
    ...
    if (winningColor) {
        ...
        fadeBackground.setFillColor(sf::Color(0, 0, 0, 150));
...
void BoardScene::drawSuggestedMove(sf::RenderWindow& window) const {
    ...
    suggestion.setFillColor(sf::Color(255, 0, 0, 128));
...
bool BoardScene::handleHistoryEvent(const std::optional<sf::Event>& event, sf::RenderWindow& window) {
    ...
    if (keyPressedEvent->code == sf::Keyboard::Key::Left) {
        ...
        lastMove.isBlack
            ? board.removeBlackStone(Board::getGlobalIndex(lastMove.pos))
            : board.removeWhiteStone(Board::getGlobalIndex(lastMove.pos));
        for (const auto& capturedIndex : lastMove.capturedIndices) {
            if (capturedIndex == -1) continue;
            lastMove.isBlack ? board.addStoneWhite(capturedIndex) : board.addStoneBlack(capturedIndex);
    ...
    if (keyPressedEvent->code == sf::Keyboard::Key::Right) {
        ...
        for (const auto& capturedIndex : nextMove.capturedIndices) {
            if (capturedIndex == -1) continue;
            nextMove.isBlack ? board.removeWhiteStone(capturedIndex) : board.removeBlackStone(capturedIndex);
...
void BoardScene::playMove(const Position pos) {
    ...
    if (CaptureService::checkCapture(board, Board::getGlobalIndex(pos), colorToPlay == sf::Color::Black, captures,
                                     count)) {
        if (captureSound) {
            captureSound->play();
        }
