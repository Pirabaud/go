...
bool CheckLegalMove::checkCaptureInDirection(Board& board, const int globalIndex,
    ...
    if (globalIndex - 1 * dir < 0 || globalIndex - 1 * dir >= (Board::SIZE * (Board::SIZE + 1)) || Board::isOutOfBounds(globalIndex, -1, dir)) return false;
    if (globalIndex + 2 * dir < 0 || globalIndex + 2 * dir >= (Board::SIZE * (Board::SIZE + 1)) || Board::isOutOfBounds(globalIndex, 2, dir)) return false;
