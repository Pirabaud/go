...
bool CaptureService::winLineBreakable(Board& board, const bool isBlack, const int startIndex, const int dirAlignment) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (const int dir : directions) {
            ...
            if (afterIndex < 0 || afterIndex >= (Board::SIZE * (Board::SIZE + 1)) || beforeIndex < 0 || beforeIndex >= (
            ...
            if (Board::isBitAt(allyBitBoard, checkStone + dir)) {
                ...
                if (
                    (isEnemyAfter && isEmptyBefore && CheckLegalMove::isLegalMove(checkStone - dir, board, !isBlack) ==
                        IllegalMoves::Type::NONE)
                    || (isEnemyBefore && isEmptyAfter && CheckLegalMove::isLegalMove(
                        checkStone + 2 * dir, board, !isBlack) == IllegalMoves::Type::NONE)
                ) {
                    return true;
                }
            }
...
int CaptureService::checkCaptureInDirection(Board& board, const int globalIndex,
    ...
    if (enemyBitBoard[firstEnemyArrayIndex] & firstEnemyMask && enemyBitBoard[secondEnemyArrayIndex] &
        ...
        if (removeSTone) {
            ...
            isBlack
                ? board.removeWhiteStone(secondEnemyGlobalIndex)
                : board.removeBlackStone(secondEnemyGlobalIndex);
