...
TEST_CASE_METHOD(CheckWinFixture, "Check Win Service") {
    SECTION("No win condition") {
        REQUIRE(CheckWinService::isWin(board) == nullptr);
    }

    SECTION("Wins horizontally") {
    ...
    SECTION("Wins diagonally (top-left to bottom-right)") {
        for (int row = 0; row < Board::SIZE - 4; ++row) {
            for (int col = 0; col < Board::SIZE - 4; ++col) {
                Board whiteBoard;
                Board blackBoard;
    ...
    SECTION("Wins diagonally (bottom-left to top-right)") {
